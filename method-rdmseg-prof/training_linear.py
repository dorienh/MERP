...
def train(train_loader, model, valid_loader, args):
    ...
    with torch.no_grad():
        ...
        for batchidx, (feature, label) in enumerate(train_loader):
            ...
            loss_mse = F.mse_loss(output.squeeze(), label.squeeze())
    ...
    for epoch in np.arange(args.num_epochs):
        ...
        # iter += 1
        if epoch > n_epochs_least and (epochs_no_improve == n_epochs_stop or epochs_no_improve > n_epochs_least+epochs_no_improve):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--conditions', nargs='+', type=str, default=['training_duration'])
