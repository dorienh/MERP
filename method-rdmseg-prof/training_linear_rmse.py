...
def train(train_loader, model, valid_loader, args):
    ...
    with torch.no_grad():
        ...
        for batchidx, (feature, label) in enumerate(train_loader):
            ...
            epoch_loss_log['rmse'].append(loss_rmse.item())
            epoch_loss_log['r'].append(loss_r.item())
        
        aveloss_rmse = np.average(epoch_loss_log['rmse'])
        aveloss_r = np.average(epoch_loss_log['r'])
        print(f'Initial round without training || rmse = {aveloss_rmse:.2f} || r = {aveloss_r:.2f}')
        loss_log['train_rmse'].append(aveloss_rmse)
    ...
    for epoch in np.arange(args.num_epochs):
        ...
        for batchidx, (feature, label) in enumerate(train_loader):
            ...
            # print('loss_r: ', loss_r)
            loss = loss_rmse*args.mse_weight + loss_r
            ...
            print(f'Epoch: {epoch} || Batch: {batchidx}/{numbatches} || rmse = {loss_rmse.item():5f} || r = {loss_r.item():5f}', end = '\r')
        ...
        aveloss_rmse = np.average(epoch_loss_log['rmse'])
        aveloss_r = np.average(epoch_loss_log['r'])
        # print(f'Initial round without training || mse = {aveloss_rmse:.2f} || r = {aveloss_r:.2f}')
        loss_log['train_rmse'].append(aveloss_rmse)
        loss_log['train_r'].append(aveloss_r)
        print(' '*200)

        epoch_duration = time.time() - start_time
        print(f'Epoch: {epoch:3} || rmse: {aveloss_rmse:8.5f} || r: {aveloss_r:8.5f} || time taken (s): {epoch_duration:8f}')
        
        # test_ave_mse, test_ave_r, sum_test  = test(model, test_loader)
        valid_ave_rmse, valid_ave_r, valid_loss  = test(model, valid_loader)

        # loss_log['test_mse'].append(test_ave_mse)
        # loss_log['test_r'].append(test_ave_r)
        loss_log['valid_rmse'].append(valid_ave_rmse)
        ...
        if valid_loss < min_val_loss:
            ...
            best_epoch = epoch
            best_model = model
            best_valid_ave_rmse = valid_ave_rmse
    ...
    plt.plot(loss_log['train_rmse'][1::], label='training loss rmse')
    plt.plot(loss_log['valid_rmse'], label='validation loss rmse')
    ...
    plt.ylim([-1, 1])
    plt.legend()
    plt.title(f"Loss || init mse:{loss_log['train_rmse'][0]:.3f} | r:{loss_log['train_r'][0]:.3f} || test rmse: {best_valid_ave_rmse:.3f} | r: {best_valid_ave_r:.3f}")
    plt.savefig(os.path.join(args.dir_path, 'saved_models', f'{args.model_name}', 'loss_plot.png'))
    plt.close()

    return best_model, best_valid_ave_rmse, best_valid_ave_r, best_epoch
...
def test(model, test_loader):
    ...
    test_ave_rmse = np.average(epoch_loss_log['rmse'])
    test_ave_r = np.average(epoch_loss_log['r'])
    # sum_test = test_ave_mse + abs(test_ave_r)
    sum_test = test_ave_rmse + test_ave_r
    print(f'average test lost (per batch): mse: {test_ave_rmse:4f} r: {test_ave_r:4f} sum: {sum_test:4f}')
    return test_ave_rmse, test_ave_r, sum_test
...
def single_test(model, songurl, feat_dict, exps, args, filename_prefix=None):
    ...
    for j in np.arange(len(testlabels)):
        ...
        # temp_plot_c = plot_pred_comparison(output, testlabels[j], np.mean(loss_rmse_list), np.mean(loss_r_list))
        c_axs[j].plot(output) #, label='prediction')
        c_axs[j].plot(testlabels[j]) #, label='ground truth')
        rloss = np.mean(loss_r_list)
        rmseloss = np.mean(loss_rmse_list)
        c_axs[j].set_title(f'rmse: {rmseloss:.5} || r: {rloss:.5}')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_name', type=str, default='rmse', help='Name of folder plots and model will be saved in')
    parser.add_argument('--batch_size', type=int, default=8)
    parser.add_argument('--num_workers', type=int, default=10)
    parser.add_argument('--hidden_dim', type=int, default=512)
    parser.add_argument('--lstm_size', type=int, default=1)
    ...
    # train_loader= dataloader_prep(test_feat_dict, exps, args)
    
    model, val_ave_rmse, val_ave_r, num_epochs = train(train_loader, model, valid_loader, args)
    ...
    test_ave_rmse, test_ave_r, sum_test  = test(model, test_loader)
    ...
    args_dict['v_rmse'] = f'{val_ave_rmse:.6f}'
    args_dict['v_r'] = f'{val_ave_r:.6f}'
    args_dict['v_loss'] = f'{val_ave_rmse+val_ave_r:.6f}'

    args_dict['t_rmse'] = f'{test_ave_rmse:.6f}'
